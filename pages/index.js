...
const IndexPage = ({ tinaContent }) => {
  const { colors } = useTheme();
  const [content] = useLocalJsonForm(tinaContent , {
    fields: [
      {
        name: "title",
        label: "Title",
  ...
  return (
    <div css={mainColumnStyles}>
      ...
      <OptInForm css={tw`h-10 mx-2 xl:mb-32 relative z-40`} title={content.title} subtitle={content.subtitle} />
    </div>
  );
...
IndexPage.getInitialProps = async ctx => {
  ...
  let content = await import(`../content/index.json`);
